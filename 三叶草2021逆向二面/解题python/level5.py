def shift_rows(a1):
    v2=a1[13]
    a1[13]=a1[9]
    a1[9]=a1[5]
    a1[5]=a1[1]
    a1[1]=v2
    v3=a1[10]
    a1[10]=a1[2]
    a1[2]=v3
    v4=a1[14]
    a1[14]=a1[6]
    a1[6]=v4
    v5=a1[3]
    a1[3]=a1[7]
    a1[7]=a1[11]
    a1[11]=a1[15]
    a1[15]=v5
def decode(cmp,v8):
    x = 22 * 16
    for k in range(22):
        shift_rows(cmp)
        for i in range(16):
            cmp[i] = sbox.index(cmp[i])
        x -= 16
        for j in range(16):
            cmp[j] ^= v11[x]
            x += 1
        x -= 16
    for i in range(16):
        cmp[i] ^= v8[i]
    return cmp
date=[0xF8, 0x88, 0x9D, 0x31, 0xC7, 0x24, 0xD5, 0xF9, 0x45, 0x28, 0x9B, 0x7D, 0x42, 0x66, 0x65, 0x29, 0x2E, 0xC3, 0xC3, 0x5F, 0x1E, 0x1C, 0x43, 0xE9, 0x1E, 0xE1, 0xB9, 0xBD, 0x85, 0x5B, 0x7A, 0x39, 0xDE, 0xF6, 0xA7, 0xB8, 0xF5, 0xD2, 0xF9, 0xFE, 0xD9, 0xA0, 0x13, 0xF3, 0x43, 0xEF, 0xB2, 0x8A]
sbox=[]
v11=[]
cmp1=date[0:16]
cmp2=date[16:32]
cmp3=date[32:48]
v8=[ord(i) for i in list('vMBixpswDVgJhpZd')]
flag1=decode(cmp1,v8)
flag2=decode(cmp2,date[0:16])
flag3=decode(cmp3,date[16:32])
flag1=''.join([chr(a) for a in flag1])
flag2=''.join([chr(a) for a in flag2])
flag3=''.join([chr(a) for a in flag3])
print(flag1+flag2+flag3)